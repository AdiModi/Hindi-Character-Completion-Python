...
def inkDropNoise(image):
    ...
    inkDropImage = cv2.warpAffine(inkDropImage, M, (pp.imageShape[0], pp.imageShape[1]))
    image = cv2.add(image, inkDropImage)
    return image

for i in range(0, 20):
    image = cv2.imread('C:\\Users\\Aditya\\Desktop\\TEST_IMG.png', cv2.IMREAD_GRAYSCALE)
    cv2.imwrite('C:\\Users\\Aditya\\Desktop\\Noisy_' + str(i+1)+ '.png', inkDropNoise(image))